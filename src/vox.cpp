...
namespace vox {
  ...
  inline bool cmp_id(const char* from, const char* to) {
    return ( (from[0] == to[0]) && (from[1] == to[1]) && (from[2] == to[2]) && (from[3] == to[3]) );
  }
  ...
  #define READ(stream, value) if (!read(stream, value)) { return false; }
  ...
  bool parse(char* fileName, vox_t& vox) {
    ...
    std::memcpy(vox.palette, default_palette, sizeof default_palette );
    ...
    vox.models.reserve(1);
  
    while (read(input, chunk)) {
      if (cmp_id(chunk.id, "MAIN")) {
        ...
        auto& model = vox.models.back();
        int num_voxels;
        READ(input, num_voxels);
        model.voxels.resize(num_voxels);
        for (int i = 0; i < num_voxels; ++i) {
          READ(input, model.voxels[i]);          
        }
      } else if (cmp_id(chunk.id, "RGBA")) {
        for (int i = 0; i < 255; ++i) {
          READ(input, vox.palette[i]);
        }
      } else if (cmp_id(chunk.id, "MATT")) {
        material_t material; 
        READ(input, material.id);
        READ(input, material.type);
        READ(input, material.weight);
        READ(input, material.property);
        int property = material.property;
        for (int i = 0; i < 7; ++i) {
          if (property & 1) {
            READ(input, material.property_value[i]);
          }
          property >>= 1;
        }
      }
